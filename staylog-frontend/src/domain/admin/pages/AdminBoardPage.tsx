...
const updateBoardStatus = async (boardId: number, status: "Y" | "N") => {
  try {
    await api.patch(`/v1/admin/boards/${boardId}/status`, { deleted: status });
    return true;
  } catch (err) {
...
function AdminBoardPage() {
  ...
  useEffect(() => {
    api
      .get<AdminBoardListResponse>("/v1/admin/boards", { params: searchParams })
      .then((res: any) => {
        // 안전 파싱(data.data | data | res)
        const root = res?.data?.data ?? res?.data ?? res;
        setBoards(Array.isArray(root?.boards) ? root.boards : []);
  ...
  const handleStatusChange = async (
    ...
    const originalStatus = boards.find((item) => item.boardId === boardId)
      ?.deleted!;
    ...
    setBoards((prev) =>
      prev.map((item) =>
        item.boardId === boardId ? { ...item, deleted: newStatus } : item
      )
    );
    ...
    if (!success) {
      ...
      setBoards((prev) =>
        prev.map((item) =>
          item.boardId === boardId ? { ...item, deleted: originalStatus } : item
        )
      );
    }
  };
        ...
        <button
          className={`btn ${
            searchParams.boardType === "BOARD_REVIEW"
              ? "select-bg border-light fw-bold"
            ...
            <select
              ...
              onChange={(e) => {
                const [sortBy, sortOrder] = e.target.value.split("_") as [
                  "createdAt" | "viewsCount" | "rating" | "likesCount",
                  "ASC" | "DESC"
                ];
                setSearchParams((prev) => ({
                  ...prev,
              ...
              <div className="card-body">
                ...
                <div className="d-flex justify-content-between align-items-start gap-2">
                  ...
                  <div className="text-nowrap">
                    <span
                      className={`badge ${
                        item.deleted === "N" ? "bg-success" : "bg-danger"
                      }`}
                    >
                      {item.deleted === "N" ? "공개" : "숨김"}
                    </span>
