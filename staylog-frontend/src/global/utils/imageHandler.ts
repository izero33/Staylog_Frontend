...
export async function handleImageUpload(quillRef: any, targetType: string, targetId: string) {
  ...
  input.addEventListener("change", async () => {
    ...
    try {
      const res = await imageUploadApi({ imgs: file, targetType, targetId });
      ...
      if (!imgUrl) {
        console.error("응답에 imageUrl이 없음:", res);
